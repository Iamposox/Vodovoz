	...
	public class OrdersReader
	{
		...
		public void Read(string xml)
		{
			...
			foreach(XmlNode node in content.SelectNodes("/КоммерческаяИнформация/Документ")) {
				...
				var paymentDateNode = requisites.SelectSingleNode("ЗначениеРеквизита[Наименование='Дата оплаты']/Значение");
				if(paymentDateNode != null && DateTime.TryParse(paymentDateNode.InnerText, out paymentDate))
